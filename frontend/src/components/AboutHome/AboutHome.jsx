            ...
            <div className='w-full order-1 lg:order-2 space-y-8 sm:space-y-12 relative'>
                <div className='space-y-4 sm:space-y-8 px-4 sm:px-0'>
                    <h2 className='text-3xl sm:text-4xl md:text-5xl xl:text-6xl font-bold leading-tight'>
                        <span className='font-cursive text-4xl sm:text-5xl md:text-6xl bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent'>
                    ...
                    <p className='text-base sm:text-lg md:text-xl opacity-90 leading-relaxed max-w-3xl font-serif italic border-l-4 border-amber-500/60 pl-4 sm:pl-6 py-2 bg-gradient-to-r from-white/5 to-transparent'>
                        "Enjoy delicious local dishes prepared with care and tradition. At P-ZEL Ghana Chop Bar, we serve you the true taste of Ghana in every bite."
                    </p>
                </div>

                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 px-4 sm:px-0'>
                    {aboutfeature.map((item, i) => (
                        <div key={i} className='flex flex-col items-center justify-center gap-3 sm:gap-4 p-4 sm:p-5 transition-transform duration-300 hover:scale-105'> 
                            <div className={`p-3 sm:p-4 bg-gradient-to-br ${item.color} rounded-full transition-all duration-300 hover:scale-110`}>
                                {item.icon && <item.icon className='text-2xl sm:text-3xl text-white' />}
                            </div>
                            <div className='text-center'>
                                <h3 className='text-xl sm:text-2xl font-bold font-cursive transition-colors duration-300 hover:text-amber-400'>{item.title}</h3>
                ...
                <div className='flex flex-wrap gap-4 items-center mt-6 sm:mt-8 px-4 sm:px-0'>
                    <Link to='/about' className='px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-amber-500 to-orange-600 rounded-xl font-bold
                    hover:scale-[1.02] transition-transform duration-300 flex items-center gap-2 sm:gap-3 shadow-xl
                    hover:shadow-amber-500/20 group relative overflow-hidden'>
                        <span className='absolute inset-0 bg-gradient-to-r from-amber-600/30 to-transparent opacity-0 group-hover:opacity-100
                        transition-opacity duration-300'>
                        </span>
                        <FaInfoCircle className='text-lg sm:text-xl animate-pulse'/>
                        <span className='font-cursive text-lg sm:text-xl'>About P-ZEL</span>
                    </Link>
                </div>
            </div>

            <div className='w-full order-2 lg:order-1 md:max-w-md lg:max-w-none lg:w-4/12 mt-12 mb-10 lg:mb-0 relative group transform
