...
const CartPage = () => {
    ...
    return (
        <div className='min-h-screen overflow-x-hidden py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d]'>
                    ...
                    <>
                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
                            {cartItems.map((cartItem) => {
                                ...
                                const usdTotal = convertToUSD(itemTotal);
                                const usdPrice = convertToUSD(itemPrice);
                                
                                return (
                                    <div key={_id} className='group bg-amber-900/20 rounded-2xl border-4 border-dashed border-amber-500 
                                    backdrop-blur-sm flex flex-col items-center gap-4 transition-all duration-300 hover:border-solid 
                                    hover:shadow-xl hover:shadow-amber-900/10 transform hover:-translate-y-1 animate-fade-in'>
                                        <div className='w-24 h-24 flex-shrink-0 cursor-pointer relative overflow-hidden rounded-lg 
                                        transition-transform duration-300 mt-4' 
                                        ...
                                        <div className='w-full text-center'>
                                            ...
                                            <p className='text-amber-100/80 font-cinzel mt-1'>
                                                LRD {itemPrice.toFixed(2)} <span className='text-amber-500 text-xs'>(${usdPrice} USD)</span>
                                            </p>
                                        </div>

                                        <div className='flex items-center gap-3'>
                                            <button onClick={() => updateQuantity(_id, Math.max(1, quantity - 1))} className='w-8 
                                            h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 
                                            ...
                                            <button onClick={() => updateQuantity(_id, quantity + 1)} className='w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-all duration-200 active:scale-95'>
                                                <FaPlus className='w-4 h-4 text-amber-100' />
                                            </button>
                                        </div>

                                        <div className='flex items-center justify-between w-full px-4 pb-4'>
                                            <button onClick={() => removeFromCart(_id)} className='bg-amber-900/40 px-3 py-1 rounded-full font-cinzel text-xs uppercase transition-all duration-300 hover:bg-amber-800/50 flex items-center gap-1 active:scale-95'>
                                ...
                                <div className='flex items-center gap-8'>
                                    <div className='text-right'>
                                        <h2 className='text-3xl font-dancingscript text-amber-100'>
                                            Total: LRD {Number(totalAmount).toFixed(2)}
                                        </h2>
                                        <p className='text-sm text-amber-500 text-right'>
                                            (${convertToUSD(totalAmount)} USD)
                                        </p>
                                    </div>
            ...
            {selectedImage && (
                <div className='fixed inset-0 z-50 flex items-center justify-center bg-amber-900/40 backdrop-blur-sm p-4 overflow-auto'
